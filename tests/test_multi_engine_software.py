...
@pytest.mark.asyncio
async def test_3dec_sel_sdk_units_resolve() -> None:
    # 7.0-only legacy SEL scripting interface, authored from live 7.00 __doc__.
    for api in ("itasca.sel", "itasca.sel.cable", "itasca.sel.cable.Cable"):
        result = await mcp.call_tool("itasca_browse_python_api", {"software": "3dec", "api": api})
        payload = _parse_tool_payload(result)
...
def test_3dec_sdk_version_availability_markers() -> None:
    ...
    sdk = RESOURCES_DIR / "3dec" / "python_sdk_docs" / "modules"
    # block.field is 9.x-only (absent from live 3DEC 7.00 introspection).
    field = json.loads((sdk / "block" / "field" / "module.json").read_text(encoding="utf-8"))
    assert field["availability"]["versions"] == ["9.0"]
    # sel units are 7.0-only.
    cable = json.loads((sdk / "sel" / "cable" / "Cable.json").read_text(encoding="utf-8"))
    assert cable["availability"]["versions"] == ["7.0"]
    # structure prop/set_prop were live-9.7 discoveries, absent in 7.0.
    beam = json.loads((sdk / "structure" / "Beam.json").read_text(encoding="utf-8"))
    marked = {m["name"]: m.get("availability") for m in beam["methods"] if m["name"] in ("prop", "set_prop")}
    assert marked == {"prop": {"versions": ["9.0"]}, "set_prop": {"versions": ["9.0"]}}
